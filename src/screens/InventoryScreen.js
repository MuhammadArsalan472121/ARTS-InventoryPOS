...
export default function InventoryScreen({ 
  ...
  // Filter for lower list search
  const filteredInventory = safeItems.filter((i) => 
    i?.name?.toLowerCase().includes(search.toLowerCase())
  );

  // Filter for quick update dropdown items
  const dropdownFilteredItems = safeItems.filter((i) =>
    i?.name?.toLowerCase().includes(dropdownSearch.toLowerCase())
  );
  ...
  const handleApplyStockUpdate = () => {
    ...
    // Reset quick selection after save
    setIsDropdownOpen(false);
  };
    ...
    <TouchableWithoutFeedback onPress={() => { closeDropdown(); Keyboard.dismiss(); }}>
      ...
      >
        ...
        <View style={styles.contentPadding}>
          ...
          <View style={[styles.quickEditCard, { zIndex: 100 }]}>
                ...
                <ScrollView style={{ maxHeight: 160 }} nestedScrollEnabled={true} keyboardShouldPersistTaps="handled">
                  {dropdownFilteredItems.length === 0 ? (
                    <View style={styles.dropdownOption}>
                      <Text style={styles.dropdownOptionText}>No matching items</Text>
                    </View>
                  ) : (
                    dropdownFilteredItems.map((item, index) => (
                      <TouchableOpacity
                        key={item.id}
                        style={[
                          styles.dropdownOption,
                          index === dropdownFilteredItems.length - 1 && { borderBottomWidth: 0 },
            ...
            return (
              <View key={item.id} style={styles.inventoryCard}>
                <View style={[styles.stockBox, { backgroundColor: isLow ? '#FEF3C7' : '#D1FAE5' }]}>
                  <Text style={[styles.stockBoxNumber, { color: isLow ? COLORS.warningYellow || '#D97706' : COLORS.successGreen || '#10B981' }]}>
                    {item.stock}
                  </Text>
                ...
                <View style={{ flex: 1, paddingHorizontal: 12 }}>
                  ...
                  <View style={[styles.miniBadge, { backgroundColor: isLow ? '#FEF3C7' : '#D1FAE5' }]}>
                    <Text style={[styles.miniBadgeText, { color: isLow ? COLORS.warningYellow || '#D97706' : COLORS.successGreen || '#10B981' }]}>
                      {isLow ? 'Low' : 'OK'}
                    </Text>
                  </View>
                </View>
          ...
          {filteredInventory.length === 0 && (
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>No items found matching "{search}"</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  statSubtextWarning: { 
    fontSize: 10, 
    color: COLORS.warningYellow || '#D97706' 
  },
  alertBanner: { 
